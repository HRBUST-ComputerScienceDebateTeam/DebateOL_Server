...
bool DB_MYSQL_OFROOM::updata_Room_base( int roomid, DAL_Room_Base t ) {
    ...
    for ( int i = 0; i < DAL_Room_Base::title_DAL_Room_Base.size(); i++ ) {
        if ( mp_modify[ DAL_Room_Base::title_DAL_Room_Base[ i ] ] == to_string( INT_DEFAULT ) || mp_modify[ DAL_Room_Base::title_DAL_Room_Base[ i ] ] == STR_DEFAULT ) {
            //没被填写
            continue;
        } else {
            // TODO 检测
...
bool DB_MYSQL_OFROOM::updata_Room_extra( int roomid, DAL_Room_Extra t ) {
    ...
    map< string, string > mp_modify = t.toMap();
    for ( int i = 0; i < DAL_Room_Extra::title_DAL_Room_Extra.size(); i++ ) {
        if ( mp_modify[ DAL_Room_Extra::title_DAL_Room_Extra[ i ] ] == to_string( INT_DEFAULT ) || mp_modify[ DAL_Room_Extra::title_DAL_Room_Extra[ i ] ] == STR_DEFAULT ) {
            //没被填写
            continue;
        } else {
            // TODO 检测
