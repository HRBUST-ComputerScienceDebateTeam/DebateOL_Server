...
std::string AudioDownload( const std::string& s ) {
    ...
    //拆解请求 - 正则优化
    bool Ok_flag = 1;
    sscanf( s.c_str(), "%d-%d-%d-%d-%d-%d-%d", &sendinfo.type, &sendinfo.roomId, &sendinfo.userId, &sendinfo.min, &sendinfo.sec, &sendinfo.msec, &sendinfo.sendtime );

    if ( sendinfo.min > 60 || sendinfo.min < 0 )
        Ok_flag = 0;
    if ( sendinfo.sec > 60 || sendinfo.sec < 0 )
        Ok_flag = 0;
    if ( sendinfo.msec > 1000 || sendinfo.msec < 0 )
        Ok_flag = 0;
    if ( sendinfo.roomId > MAX_ROOM || sendinfo.roomId < 0 )
        Ok_flag = 0;
    if ( sendinfo.userId > MAX_USER || sendinfo.userId < 0 )
        Ok_flag = 0;
    if ( sendinfo.sendtime > 3600000 || sendinfo.sendtime < 0 )
        Ok_flag = 0;
