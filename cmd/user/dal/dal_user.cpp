...
bool DB_MYSQL_OFUSER::updata_user_base( int uid, DAL_User_Base t ) {  //整表更改
    ...
    for ( int i = 0; i < DAL_User_Base::title_DAL_User_Base.size(); i++ ) {
        if ( mp_modify[ DAL_User_Base::title_DAL_User_Base[ i ] ] == INT_DEFAULT || mp_modify[ DAL_User_Base::title_DAL_User_Base[ i ] ] == STR_DEFAULT ) {
            //没被填写
            continue;
        } else {
            // TODO 检测
...
bool DB_MYSQL_OFUSER::updata_user_extra( int uid, DAL_User_Extra t ) {
    ...
    map< string, string > mp_modify = t.toMap();
    for ( int i = 0; i < DAL_User_Extra::title_DAL_User_Extra.size(); i++ ) {
        if ( mp_modify[ DAL_User_Extra::title_DAL_User_Extra[ i ] ] == INT_DEFAULT || mp_modify[ DAL_User_Extra::title_DAL_User_Extra[ i ] ] == STR_DEFAULT ) {
            //没被填写
            continue;
        } else {
            // TODO 检测
...
bool DB_MYSQL_OFUSER::updata_user_social( int uid, DAL_User_Social t ) {
    ...
    map< string, string > mp_modify = t.toMap();
    for ( int i = 0; i < DAL_User_Social::title_DAL_User_Social.size(); i++ ) {
        if ( mp_modify[ DAL_User_Social::title_DAL_User_Social[ i ] ] == INT_DEFAULT || mp_modify[ DAL_User_Social::title_DAL_User_Social[ i ] ] == STR_DEFAULT ) {
            //没被填写
            continue;
        } else {
            // TODO 检测
...
bool DB_MYSQL_OFUSER::updata_UserUUlevel( int uidA, int uidB, int newlevel ) {
    ...
    string sql = "update DAL_UU_relation set UULevel  = " + to_string( newlevel ) + " where UseridA = " + to_string( uidA ) + " and UseridB = " + to_string( uidB ) + ";";
    DBRES* res = DB_MYSQL_OFUSER::DB_mysql.exec( sql );
    if ( res->checkerr( sql ) == false ) {
        cout << __func__ << " err " << endl;
        return false;
