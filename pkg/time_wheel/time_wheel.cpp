...
Time_Wheel::WORK::WORK( int index, int time, void* ( *workfn )( void* ), void* arg ) {
    this->index  = index;
    this->workfn = workfn;
    this->arg    = arg;
    set_time( time );
...
void Time_Wheel::add_work( int index, int time, void* ( *workfn )( void* ), void* arg ) {
    //添加任务
...
bool Time_Wheel::find_work( int index ) {
    return ( work_mp.find( index ) != work_mp.end() );
}
int Time_Wheel::size() {
    return work_mp.size();
}